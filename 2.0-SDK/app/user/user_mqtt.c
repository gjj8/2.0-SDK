...
void  ICACHE_FLASH_ATTR mqttDataCb(uint32_t *args, const char* topic, uint32_t topic_len, const char *data, uint32_t data_len)
{
	...
	if(1)
	{
		...
		else if(IsStartWith("ReadTest:", dataBuf))
		{
			INFO("MQTT: Receive MeterData Requestion!!!\r\n");
			MqttOK = 2;
			uart0_tx_buffer(bufMeter, 8);
			...
			os_bzero(mqtt_topic_send, 16);
			os_sprintf(mqtt_topic_send, "%s", dataBuf+9);
		...
		else if(IsStartWith("upgrade", dataBuf))
		{
			...
			MQTT_Publish(client, "/meter/upgrade", temp, strlen(temp), 0, 0);
		...
		{
			MQTT_Publish(client, "/meter/info", mqtt_id, 12, 0, 0);
			system_restart();
			return;
		}
